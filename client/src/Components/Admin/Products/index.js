...
class AdminProducts extends Component {
    ...
    updateStatus(product) {
        ...
        httpRequest
            .post(`/admin/update-product?_id=${product._id}`, { status : !productStatus })
            .then((response) => {
                if (response.data) {
                    this.props.updateAdminProducts(response.data);
                }
            });
    }
    ...
    deleteProduct(product) {
        httpRequest
            .get(`/admin/delete-product?_id=${product._id}`)
            .then((response) => {
                if (response.data) {
                    this.props.updateAdminProducts(response.data);
                }
        ...
        return this.props.products.loading ? (
            ...
            <div>
                <div className="row row-col-1 row-cols-md-2">
                    {_.map(
                        this.props.products.adminProducts,
                        (product, idx) => (
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        updateAdminProducts: (updatedAdminProducts) =>
            dispatch(fetchAdminProductSuccess(updatedAdminProducts)),
    };
};
